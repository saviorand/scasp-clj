...
(defn- print-help []
  ...
  (println "    \"text\"     = strings    (\"hello world\")")
  (println "    not term   = negation-as-failure")
  (println "    Comparisons: <  >  <=  >=  ==  !=")
  (println "    Arithmetic:  X is Y + 1")
  (println)
  (println "  Effects:")
  (println "    print(X), println(X), nl          # output")
  (println "    read_line(X), read_number(X)      # input")
  (println "    string_concat(A, B, C)            # C = A ++ B")
  (println "    write_file(Path, Content)          # file I/O")
  (println "    read_file(Path, Content)")
  (println)
  (println "  Commands:")
